...
const USER_AGENT: &str = concat!("profound/", env!("CARGO_PKG_VERSION"));
...
impl Environment {
    ...
    pub fn base_url(self) -> &'static str {
        match self {
            Self::Production => "https://api.tryprofound.com",
            Self::Development => "https://dev.api.tryprofound.com",
        }
    }
}
...
/// Asynchronous client for the ProfoundClient API.
///
/// Cloning is cheap: all state is shared behind an `Arc`. Construct one with
/// [`ProfoundClient::builder`] or [`ProfoundClient::from_env`].
#[derive(Clone)]
pub struct ProfoundClient {
    inner: Arc<Inner>,
}

impl ProfoundClient {
    /// Starts building a client with the fluent builder.
    pub fn builder() -> ProfoundClientBuilder {
        ProfoundClientBuilder::new()
    }
    ...
    pub fn from_env() -> Result<Self, Error> {
        ...
        if let Some(value) = read_env("PROFOUND_BASE_URL") {
            builder = builder.base_url(value);
        }
        if let Some(value) = read_env("PROFOUND_API_KEY") {
            builder = builder.api_key(value);
        }
        if let Some(value) = read_env("PROFOUND_ACCESS_TOKEN") {
...
impl std::fmt::Debug for ProfoundClient {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ProfoundClient")
...
impl ProfoundClient {
    ...
    /// Cap on buffered response bodies, exposed to generated streaming calls
    /// so the error-path buffer honors the configured limit (the success path
    /// is framed incrementally and exempt from the cap).
    pub(crate) fn max_response_size(&self) -> usize {
        self.inner.max_response_size
    }

    /// Pairs the client's sleeper with a builder-configured per-event idle
    /// timeout. Streaming uses the same timer the retry/deadline paths do, so
    /// a BYO-transport client on a non-tokio runtime enforces it too.
    pub(crate) fn idle_timeout(&self, event_timeout: std::time::Duration) -> crate::streaming::IdleTimeout {
        crate::streaming::IdleTimeout::new(self.inner.sleep.clone(), event_timeout)
    }
...
/// Fluent builder for [`ProfoundClient`].
#[derive(Default)]
pub struct ProfoundClientBuilder {
...
impl ProfoundClientBuilder {
    ...
    pub fn build(self) -> Result<ProfoundClient, Error> {
...
impl std::fmt::Debug for ProfoundClientBuilder {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ProfoundClientBuilder")
