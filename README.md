...
This library provides convenient access to the profound REST API from asynchronous Rust.
...
```toml
[dependencies]
profound = "0.2.0" # x-release-please-version
...
```rust,no_run
use profound::ProfoundClient;

async fn run() -> Result<(), Box<dyn std::error::Error>> {
    let client = ProfoundClient::builder().api_key("…").access_token("…").build()?;
...
- `api_key` — environment variable `PROFOUND_API_KEY`
- `access_token` — environment variable `PROFOUND_ACCESS_TOKEN`

## Streaming

Streaming operations return a stream of decoded events from `send().await`:

```rust,ignore
use futures::StreamExt;

let mut events = client.prompts().stream_answers_v2("…").send().await?;
while let Some(event) = events.next().await {
    let event = event?;
}
```

A complete runnable consumer ships in [`examples/streaming.rs`](./examples/streaming.rs).
...
```rust,no_run
use profound::ProfoundClient;
use profound::transport::ReqwestTransport;
...
```sh
cargo add profound --no-default-features --features tokio
```
...
```sh
cargo add --dev profound --features mock
```

```rust,ignore
use profound::transport::{InstantSleep, MockTransport};
...
```sh
cargo add profound --features tracing
```
...
```sh
RUST_LOG=profound=debug cargo run
```
...
```sh
cargo add profound --no-default-features --features reqwest,native-tls
```

## Reusing the client

Construct one `ProfoundClient` and share it for the lifetime of your
